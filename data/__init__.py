...
def define_dataloader(logger, opt):
    ...
    dataloader = DataLoader(phase_dataset, sampler=data_sampler, worker_init_fn=worker_init_fn,
                            **dataloader_args)
...
def define_dataset(logger, opt):
    ...
    dataset_opt['args']['pattern'] = opt['model']['which_networks'][0]['args']['pattern']
    dataset_opt['args']['phase'] = opt['phase']
    dataset_opt['args']['batch_size'] = opt['datasets'][opt['phase']]['dataloader']['args']['batch_size']

    phase_dataset = init_obj(dataset_opt, logger, default_file_name='data.dataset',
                             init_type='Dataset')
    val_dataset = None
