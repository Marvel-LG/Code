...
class UnPairOldPhotos_SR(BaseDataset):  ## Synthetic + Real Old
    def initialize(self, root):
        ...
        self.load_img_dir_clean = os.path.join(self.dir_AB, "VOC2012")
    ...
    def __getitem__(self, index):
        ...
        if P >= 0 and P < 1:
            ...
            is_real_old = 1
        if P >= 1 and P < 2:
            sampled_dataset = self.loaded_imgs_clean
        ...
        A_transform = get_transform(transform_params)
...
class PairOldPhotos(BaseDataset):
    ...
    def __getitem__(self, index):
        if self.opt.isTrain:
            img_name_clean, B = self.loaded_imgs_clean[index]
    ...
    def __len__(self):
        ...
        if self.opt.isTrain:
            return len(self.loaded_imgs_clean)
        else:
