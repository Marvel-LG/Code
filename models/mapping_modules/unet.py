...
from models.attention.MFEB import MFEblock
import functools

from models.mapping_modules.nn import (
    checkpoint,
    zero_module,
    normalization,
    count_flops_attn,
...
class TdResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        self.emb_channels = emb_channels  # 编码通道数
        self.dropout = dropout
        self.out_channel = out_channel or channels  # 记录输出通道数
        self.use_conv = use_conv
        self.use_checkpoint = use_checkpoint
        self.use_scale_shift_norm = use_scale_shift_norm
        ...
        if up:
            ...
            self.h_upd = Downsample(channels, False)  # h和w减半
            self.x_upd = Downsample(channels, False)
        else:
            self.h_upd = self.x_upd = nn.Identity()  # 不作任何操作

        self.emb_layers = nn.Sequential(
            SiLU(),
            nn.Linear(
                emb_channels,  # 编码输入通道
                2 * self.out_channel if use_scale_shift_norm else self.out_channel,  # 改变输出通道数
            ),
        )
        self.out_layers = nn.Sequential(
        ...
        if self.out_channel == channels:  # 输入和输出不同
            self.skip_connection = nn.Identity()
        elif use_conv:
            self.skip_connection = nn.Conv2d(
                channels, self.out_channel, 3, padding=1
    ...
    def _forward(self, x, emb):
        if self.updown:
            ...
            h = self.in_layers(x)
        emb_out = self.emb_layers(emb).type(h.dtype)
        while len(emb_out.shape) < len(h.shape):  # 扩充编码长度
            emb_out = emb_out[..., None]
        if self.use_scale_shift_norm:
            out_norm, out_rest = self.out_layers[0], self.out_layers[1:]
            scale, shift = torch.chunk(emb_out, 2, dim=1)
...
class UNet(nn.Module):
    ...
    def __init__(
            ...
            for _ in range(res_blocks):
                ...
                if ds in attn_res:
                    layers.append(
                        ...
                        # ),
                        TransformerBlock(dim=ch, num_heads=ch//num_head_channels, ffn_expansion_factor=ffn_expansion_factor,
                                         bias=False,
        ...
        self.middle_block = EmbedSequential(
            ...
            #     use_new_attention_order=use_new_attention_order,
            # ),
            TransformerBlock(dim=ch, num_heads=ch//num_head_channels, ffn_expansion_factor=ffn_expansion_factor, bias=False,
                             LayerNorm_type='WithBias'),
            ResBlock(
                ch,
                cond_embed_dim,
        ...
        for level, mult in list(enumerate(channel_mults))[::-1]:
            for i in range(res_blocks + 1):
                ...
                self._feature_size += ch
        self.mapping_out_block = EmbedSequential(
            ResBlock(
                ch,
                cond_embed_dim,
                dropout,
            ...
            ResBlock(
                ...
                cond_embed_dim,
                dropout,
                out_channel=upsample_channle,
                use_checkpoint=use_checkpoint,
                use_scale_shift_norm=use_scale_shift_norm,
            ),
        )
        self.unsample_block = nn.ModuleList([])
        # self.skip_block = nn.ModuleList([])
        ch = upsample_channle
        for level in range(self.encoder_len):
            for i in range(res_blocks + 1):
                layers = [
                    ResBlock(
                        ch,
                        cond_embed_dim,
                        dropout,
                        out_channel=int(upsample_channle * pow(1 / 2, level)),
                        use_checkpoint=use_checkpoint,
                        use_scale_shift_norm=use_scale_shift_norm,
                    )
                ]
                ch = int(upsample_channle * pow(1 / 2, level))
                # self.skip_block.append(
                #     TransformerBlock(dim=in_channel, num_heads=ch//num_head_channels, ffn_expansion_factor=ffn_expansion_factor,
                #                      bias=False,
                #                      LayerNorm_type='WithBias'))
                if ds in attn_res:
                    layers.append(
                ...
                if level < self.encoder_len - 1 and i == res_blocks:
                    out_ch = ch
                    layers.append(
                        ResBlock(
                            ch,
                    ...
                    ds *= 2
                self.unsample_block.append(EmbedSequential(*layers))
                self._feature_size += ch
    ...
    def forward(self, x, gammas, mask):
        """
        ...
        for module in self.input_blocks:
            # if isinstance(module, Patch_Attention_4):
            #     h = module(h, mask)
            # else:
            #     h = module(h, emb)
            #     hs.append(h)
        ...
        for i, module in enumerate(self.output_blocks):
            hs_i = hs.pop()
            h = torch.cat([h, hs_i], dim=1)
            h = module(h, emb)
        h = self.mapping_out_block(h, emb)
        for index, module in enumerate(self.unsample_block):
            # hs_up_i = hs_up[index]
            # hs_up_i = self.skip_block[index](hs_up_i)
            # h = torch.cat([h, hs_up_i], dim=1)
            h = module(h, emb)
        h = h.type(x.dtype)
        return self.out(h)
...
if __name__ == '__main__':
    # from models.VAE.autoencoder.autoencoder import AautoencoderKL
    from models.VAE.encode_a_model.unet import UNet as AUNet
    ...
    gammas = torch.ones((1,))
    hidden, hs = encoder_A.encodeing(input, gammas=gammas, imgc=label)

    output = model(hidden, hs, gammas, mask)
    print(output.size())
    # torch.save(model.state_dict(), '../../checkpoint/MAP_model/alatest_Network_MAP.pth')
    # print(output.size())

    # ####################################
    #     # 加载预训练模型参数的路径
    pretrained_model_path = '/home/lwg/Code/Pallette/checkpoint/MAP_model/latest_Network_MAP.pth'
    ...
    torch.save(model.state_dict(), 'latest_Network_MAP.pth')
    print("Loaded matching parameters successfully.")
# # #########################################
