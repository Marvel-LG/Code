...
from models.detection.sync_batchnorm import DataParallelWithCallback
from models.detection.antialiasing import Downsample
...
if __name__ == "__main__":
    ...
    # checkpoint_path='1_Network_D.pth'
    # checkpoint = torch.load(checkpoint_path, map_location="cpu")
    # model.load_state_dict(checkpoint, strict=False)
    model.to(device)
    input = torch.rand(1, 3, 256, 512).to(device)
    output = model(input)
    print(output.size())
    ...
    #
    # pretrained_model_path = 'FT_Epoch_latest.pt'
    # checkpoint = torch.load(pretrained_model_path)['model_state']
    # # 获取当前模型的状态字典
    ...
    # torch.save(model.state_dict(), '1_Network_D.pth')
