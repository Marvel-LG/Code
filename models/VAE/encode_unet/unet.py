...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        cond_embed_dim = embed_dim# * max(channel_mults)
        self.embedding = spectral_norm(nn.Embedding(num_embeddings=3, embedding_dim=inner_channel))
    ...
    def encodeing(self, x, imgc=None):
        ...
        for module in self.input_blocks:
            h = module(h, emb)
        h = self.middle_block_in(h, emb)
        return h

    def decodeing(self, h, imgc=None):
        emb = self.get_embedding(imgc=imgc)
        h = self.middle_block_out(h, emb)
...
if __name__ == '__main__':
    ...
    # # emb = torch.ones((b, )).to(device)
    # emb = label = torch.tensor([0]).long().to(device)
    # out = model(x, emb)
    # print(out.shape)
    ...
    # torch.save(model.state_dict(), '1_Network_MAP.pth')
