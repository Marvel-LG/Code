...
from models.VAE.encode_a_model.nn import (
    checkpoint,
    zero_module,
    normalization,
    count_flops_attn,
    ...
    def forward(self, x, time_emb, class_emb):
        for layer in self:
            if isinstance(layer, ResBlock):
                x = layer(x, time_emb)
            elif isinstance(layer, ClassResBlock):
                x = layer(x, class_emb)
            else:
                x = layer(x)
        return x
...
class ClassResBlock(EmbedBlock):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of embedding channels.
...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv):
        ...
        weight = torch.einsum(  # Q*K
            "bct,bcs->bts", q * scale, k * scale
        )  # More stable with f16 than dividing afterwards
        weight = torch.softmax(weight.float(), dim=-1).type(weight.dtype)
        a = torch.einsum("bts,bcs->bct", weight, v)  # softmax(Q*K)*V
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads_upsample = num_heads_upsample  # 上采样个数

        cond_embed_dim = inner_channel * 4
        self.class_embedding = spectral_norm(nn.Embedding(num_embeddings=3, embedding_dim=inner_channel))
        self.cond_class_embed = nn.Sequential(
        ...
        self.skip_blocks.append(nn.Conv2d(ch, embed_dim, kernel_size=1, padding=0))
        ds = 1
        ...
        self.middle_block_in = EmbedSequential(
            ResBlock(
                ch,
                cond_embed_dim,
                dropout,
        ...
        self.middle_block_out = EmbedSequential(
            ResBlock(
                embed_dim,
                cond_embed_dim,
                dropout,
    ...
    def encodeing(self, x, gammas=None, imgc=None):
        hs = []
        time_emb, class_emb = self.get_embedding(gammas=gammas, imgc=imgc)
        h = x.type(torch.float32)
        for module, skip in zip(self.input_blocks, self.skip_blocks):
            h = module(h, time_emb, class_emb)
            hs.insert(0, skip(h))
        h = self.middle_block_in(h, time_emb, class_emb)
        return h, hs

    def decodeing(self, h, hs, gammas=None, imgc=None):
        time_emb, class_emb = self.get_embedding(gammas=gammas, imgc=imgc)
        h = self.middle_block_out(h, time_emb, class_emb)
    ...
    def forward(self, x=None, gammas=None, imgc=None, *args, **kwargs):
        """
        ...
        hidden, hs = self.encodeing(x, gammas=gammas, imgc=imgc)
        out = self.decodeing(hidden, hs, gammas=gammas, imgc=imgc)  # h
        return out

    def get_class_embedding(self, imgc=None, *args, **kwargs):
        if imgc is not None:
            imgc = imgc.view(-1, )
            embedded = self.cond_class_embed(self.class_embedding(imgc))
            # print("embedded", embedded)
        else:
            embedded = None
        return embedded

    def get_time_embedding(self, imgc=None, *args, **kwargs):
        if imgc is not None:
            imgc = imgc.view(-1, )
            embedded = self.cond_embed(gamma_embedding(imgc, self.inner_channel))
        else:
            embedded = None
        return embedded

    def get_embedding(self, gammas=None, imgc=None, *args, **kwargs):
        if gammas is not None and imgc is not None:
            time_emb = self.get_time_embedding(gammas, *args, **kwargs)
            # class_emb = self.get_time_embedding(gammas, *args, **kwargs)
            class_emb = self.get_class_embedding(imgc, *args, **kwargs)
        else:
            time_emb = None
            class_emb = None
...
if __name__ == '__main__':
    ...
    gammas = torch.ones((b,)).int()
    imgc = torch.tensor([0])
    out = model(x, gammas, imgc)
    print(out.shape)

    pretrained_model_path = '/home/lwg/Code/Pallette/checkpoint/A_model/latest_Network_A.pth'
    # pre_dict = torch.load(pretrained_model_path)
    ...
    torch.save(model.state_dict(), 'latest_Network_A.pth')
