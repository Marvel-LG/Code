...
def init_obj(opt, logger, *args, default_file_name='default file', given_module=None, init_type='Network', **modify_kwargs):
    ...
    try:
        ...
        if isinstance(attr, type):
            ...
            # ret = attr
        logger.info('{} [{:s}() form {:s}] is created.'.format(init_type, class_name, file_name))#记录日志
    except:
...
def parse(args):
    ...
    if args.debug:
        ...
        opt['name'] = '{}_{}_{}'.format(opt['phase'], opt['model']['which_networks'][0]['args']['pattern'], opt['name'])
    ...
    for name in os.listdir('.'):
        if name in ['config', 'models', 'core', 'slurm', 'data', 'utils']:
